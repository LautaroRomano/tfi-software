...
type PropsType = {
  data: null | Patient;
  handleChangeData: (name: keyof Patient, value: string) => void;
  save: (data: Patient) => void;
  close: () => void;
  labels: { [key in keyof Patient]?: string };
};
  ...
  return (
    <div className="flex absolute top-0 left-0 w-screen h-screen justify-center items-center z-50 bg-[#0005]">
      <div className="flex flex-col gap-4 p-6 border border-gray-300 rounded-lg shadow-md bg-white max-w-4xl">
        ...
        {(Object.keys(labels) as (keyof Patient)[]).map((key) => (
            <div key={key} className="">
              ...
              <Input
                ...
                id={key}
                value={data[key as keyof Patient] || ""}
                onChange={(e) =>
                  handleChangeData(key as keyof Patient, e.target.value)
                }
